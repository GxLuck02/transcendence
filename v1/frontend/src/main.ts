...
class Router {
    ...
    content.innerHTML = `
      <div class="game-page">
        ...
        <div id="game-container" style="display: none; text-align: center; margin-top: 2rem;">
          <canvas id="pongCanvas" width="800" height="600" style="border: 2px solid #00d4ffff; background: #000;"></canvas>
  ...
  private startPongGame(mode: 'vs_local' | '2p_local' | 'vs_ai' | '2p_remote', difficulty?: 'easy' | 'medium' | 'hard'): void {
    ...
    const player1Name = currentUser?.username || currentUser?.display_name || 'Player 1';
    let player2Name = 'Player 2';
    
    if (mode === 'vs_ai' && difficulty) {
      const difficultyLabels = {
        easy: 'Facile',
        medium: 'Moyen',
        hard: 'Difficile'
      };
      player2Name = `IA (${difficultyLabels[difficulty]})`;
    }
    ...
    this.currentPongGame = new PongGame('pongCanvas', {
      ...
      onGameOver: (result) => {
        // L'overlay de fin de partie est géré directement dans PongGame
        console.log(`Game Over: ${result.winner} wins! Score: ${result.player1Score} - ${result.player2Score}`);
      }
    });
